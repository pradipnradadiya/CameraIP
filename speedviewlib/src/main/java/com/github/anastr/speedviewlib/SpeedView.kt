...
class SpeedView @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : Speedometer(context, attrs, defStyleAttr) {
    ...
    override fun updateBackgroundBitmap() {
        ...
        c.drawArc(speedometerRect, getStartDegree().toFloat(), (getEndDegree() - getStartDegree()) * getAverageHighSpeedOffset(), false, speedometerPaint)
        speedometerPaint.color = getAverageSpeedColor()
        c.drawArc(speedometerRect, getStartDegree().toFloat(), (getEndDegree() - getStartDegree()) * getAverageSpeedOffset(), false, speedometerPaint)
