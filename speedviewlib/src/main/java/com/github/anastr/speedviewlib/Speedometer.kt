...
abstract class Speedometer @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : Gauge(context, attrs, defStyleAttr) {
    ...
    private var averageSpeedColor = -0xff0100
    private var averageHighSpeedColor = -0x100
    private var highSpeedColor = -0x1000
     ...
     */
    protected var initTickPadding = 10f
    private var tickPadding = (getSpeedometerWidth() + dpTOpx(10f)).toInt()
    ...
    private fun initAttributeSet(context: Context, attrs: AttributeSet?) {
        ...
        averageSpeedColor = a.getColor(R.styleable.Speedometer_sv_averageSpeedColor, averageSpeedColor)
        averageHighSpeedColor = a.getColor(R.styleable.Speedometer_sv_averageHighSpeedColor, averageHighSpeedColor)
        ...
        maxGaugeValue= a.getFloat(R.styleable.Speedometer_sv_maxGaugeValue, _maxGaugeValue)
    ...
    open fun getAverageSpeedColor(): Int {
        return averageSpeedColor
    }

    open fun setAverageSpeedColor(averageSpeedColor: Int) {
        this.averageSpeedColor = averageSpeedColor
        if (!isAttachedToWindow)
            return
        updateBackgroundBitmap()
        invalidate()
    ...
    open fun getAverageHighSpeedColor(): Int {
        return averageHighSpeedColor
    }

    open fun setAverageHighSpeedColor(averageHighSpeedColor: Int) {
        this.averageHighSpeedColor = averageHighSpeedColor
        if (!isAttachedToWindow)
            return
        updateBackgroundBitmap()
        invalidate()
