...
abstract class Gauge constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : View(context, attrs, defStyleAttr) {
    ...
    /** low speed area  */
    private var lowSpeedPercent = 10.1F
    /** medium speed area  */
    private var mediumSpeedPercent = 20.0F
    /** average speed area  */
    private var averageSpeedPercent = 30.0F
    ...
    fun setGaugeMaxSpeed(maxSpd: Float)
    {
        this.maxGaugeSpeed = maxSpd;
        invalidate();
    }
    ...
    private fun  initAttributeSet(context: Context, attrs: AttributeSet?) {
        ...
        maxGaugeSpeed = a.getFloat(R.styleable.Gauge_sv_maxSpeed, maxGaugeSpeed)
        ...
        lowSpeedPercent = a.getFloat(R.styleable.Gauge_sv_lowSpeedPercent, lowSpeedPercent)
        mediumSpeedPercent = a.getFloat(R.styleable.Gauge_sv_mediumSpeedPercent, mediumSpeedPercent)
        averageSpeedPercent = a.getFloat(R.styleable.Gauge_sv_averageSpeedPercent, averageSpeedPercent)
        averageHighSpeedPercent = a.getFloat(R.styleable.Gauge_sv_averageHighSpeedPercent, averageHighSpeedPercent)
    ...
    private fun checkSpeedometerPercent() {
        ...
        require(!(averageSpeedPercent > 100 || averageSpeedPercent < 0)) { "averageSpeedPercent must be between [0, 100]" }
        require(!(averageHighSpeedPercent > 100 || averageHighSpeedPercent < 0)) { "averageSpeedPercent must be between [0, 100]" }
    }
     ...
     */
    protected fun getMaxSpeedText() = "%.${tickTextFormat}f".format(locale, maxGaugeSpeed)
     ...
     */
    fun getPercentSpeed(): Float = (currentSpeed - minSpeed) * 100f / (maxGaugeSpeed - minSpeed)

    /**
     * @return offset speed, between [0,1].
     */
    fun getOffsetSpeed(): Float = (currentSpeed - minSpeed) / (maxGaugeSpeed - minSpeed)
    ...
    fun getAverageSpeedOffset(): Float = averageSpeedPercent * .01f

    fun getAverageHighSpeedOffset(): Float = averageHighSpeedPercent * .01f
     ...
     */
    fun isInLowSection() = (maxGaugeSpeed - minSpeed) * getLowSpeedOffset() + minSpeed >= currentSpeed
     ...
     */
    fun isInMediumSection() = (maxGaugeSpeed - minSpeed) * getMediumSpeedOffset() + minSpeed >= currentSpeed && !isInLowSection()

    /**
     * @return true if current speed in High Section
     * , and it is not in Low Section or Medium Section.
     */
    fun isInHighSection() = currentSpeed > (maxGaugeSpeed - minSpeed) * getMediumSpeedOffset() + minSpeed
    ...
    fun setSpeedAt(speed: Float) {
        var newSpeed = speed
        newSpeed = if (newSpeed > maxGaugeSpeed) maxGaugeSpeed else if (newSpeed < minSpeed) minSpeed else newSpeed
    ...
    protected fun tremble() {
        ...
        mad = when {
            speed + mad > maxGaugeSpeed -> maxGaugeSpeed - speed
    ...
    private fun getSpeedValue(percentSpeed: Float): Float {
        ...
        return percentSpeed_ * (maxGaugeSpeed - minSpeed) * .01f + minSpeed
    }
    ...
    fun setMinSpeed(minSpeed: Float) {
        setMinMaxSpeed(minSpeed, maxGaugeSpeed)
    }
     ...
     */
    fun setLowSpeedPercent(lowSpeedPercent: Float) {
     ...
     */
    fun getMediumSpeedPercent(): Float {
     ...
     */
    fun setMediumSpeedPercent(mediumSpeedPercent: Float) {
    ...
    fun getAverageSpeedPercent(): Float {
        return averageSpeedPercent
    }

    fun setAverageSpeedPercent(averageSpeedPercent: Float) {
        this.averageSpeedPercent = averageSpeedPercent
        checkSpeedometerPercent()
        if (!attachedToWindow)
            return
        updateBackgroundBitmap()
    ...
    fun getAverageHighSpeedPercent(): Float {
        return averageHighSpeedPercent
    }

    fun setAverageHighSpeedPercent(averageHighSpeedPercent: Float) {
        this.averageHighSpeedPercent = averageHighSpeedPercent
        checkSpeedometerPercent()
        if (!attachedToWindow)
            return
        updateBackgroundBitmap()
